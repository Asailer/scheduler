...
void do_stat(struct jobcmd statcmd)
{
	struct waitqueue *p;
	struct stat statbuf;
	char timebuf[BUFLEN],writebuf[1024];
	int len;
	int fd;
	/*
	*打印所有作业的统计信息:
	...
	*/
	...
	if(stat("/tmp/state",&statbuf)==0){
		/* 如果FIFO文件存在,删掉 */
		if(remove("/tmp/state")<0)
			error_sys("remove file state failed");
	}

	if(mkfifo("/tmp/state",0666)<0)
		error_sys("mkfifo state failed");

	if((fd=open("/tmp/state",O_WRONLY))<0)
		error_sys("open fifo state failed");

	if(current){
		strcpy(timebuf,ctime(&(current->job->create_time)));
		timebuf[strlen(timebuf)-1]='\0';
		len=sprintf(writebuf,"%d\t%d\t%d\t%d\t%d\t%s\t%s\n",
				current->job->jid,
				current->job->pid,
				current->job->ownerid,
